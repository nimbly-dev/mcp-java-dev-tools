...
const recipeGenerateDomain = require("../../../../../../tools/features/route-synthesis/shared/recipe_generation");
...
test("[UT][transport][project_selector_contract] route_synthesis create_recipe fails closed when classHint is not an FQCN", async () => {
  ...
  assert.equal(
    out.structuredContent.projectRoot,
    ...
    recipeGenerateDomain.generateRecipe = async () => ({
      ...
      executionPlan: {
        ...
        probeCallPlan: {
          ...
          verificationMethod: "probe_wait_for_hit",
          actuated: false,
          byTool: { probe_reset: 1, probe_wait_for_hit: 1, probe_get_status: 0, probe_enable: 0 },
        },
      },
      resultType: "recipe",
      status: "single_line_probe_ready",
