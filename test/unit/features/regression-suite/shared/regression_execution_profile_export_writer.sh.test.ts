...
test("[UT][regression-suite][regression_execution_profile_export_writer_sh] exportExecutionProfileSh supports array-index bodyJson extract paths in replay helpers", async () => {
  ...
  try {
    ...
    writePlanArtifact(root, projectName, "plan-a", {
      targets: [{ type: "class_method", selectors: { fqcn: "x.A", method: "m" } }],
      prerequisites: [
        { key: "primaryName", required: false, secret: false, provisioning: "user_input" },
      ],
      steps: [
        {
          order: 1,
...
test("[UT][regression-suite][regression_execution_profile_export_writer_sh] exportExecutionProfileSh writes sh export as one-off artifact under exports date-uuid folder", async () => {
  ...
  try {
    ...
    const out1 = await exportExecutionProfileSh({
      workspaceRootAbs: root,
      exportId: "session-uniq",
    });
    const out2 = await exportExecutionProfileSh({
      workspaceRootAbs: root,
      exportId: "session-uniq",
    });
...
test("[UT][regression-suite][regression_execution_profile_export_writer_sh] exportExecutionProfileSh emits docker teardown trap when runtime context opts into autoStopOnFinish", async () => {
  ...
  try {
    ...
    assert.match(
      script,
      /docker compose -f "\$\{__MCPJVM_WORKSPACE_ROOT\}\/docker\/docker-compose-all\.yml" up -d/,
    );
    assert.match(script, /__mcpjvm_runtime_teardown\(\)/);
    assert.match(
      script,
      /docker compose -f "\$\{__MCPJVM_WORKSPACE_ROOT\}\/docker\/docker-compose-all\.yml" down/,
    );
    assert.match(
      script,
  ...
  try {
    ...
    await writeExecutionProfileExport({
      ...
      planRuns: [
        {
          order: 1,
          planName: "course-composite-service-regression-spec",
          status: "executed",
          runStatus: "pass",
          runId: "run-a",
        },
      ...
      steps: [
        {
          ...
          protocol: "http",
          transport: {
            http: { method: "GET", pathTemplate: "${targetBaseUrl}/api/metrics/hello" },
          },
          expect: [
            { id: "e1", actualPath: "response.statusCode", operator: "numeric_gte", expected: 200 },
          ],
    ...
    writePlanArtifact(root, projectName, path.basename(planRoot), {
      ...
      steps: [
        ...
        },
        {
          ...
          protocol: "http",
          transport: {
            http: { method: "POST", pathTemplate: "/owners", body: { firstName: "Ana" } },
          },
          expect: [
        ...
        },
        {
          ...
          expect: [
            { id: "e3", actualPath: "response.statusCode", operator: "numeric_gte", expected: 200 },
          ],
        },
      ],
    });
...
test("[UT][regression-suite][regression_execution_profile_export_writer_sh] exportExecutionProfileSh bundles shared profile scripts and export-local project env", async () => {
  ...
  try {
    ...
    assert.match(
      script,
      /"\$\{__MCPJVM_EXPORT_SCRIPT_DIR\}\/scripts\/keycloak-token-bootstrap\/refresh-keycloak-token\.ps1"/,
    );
