...
import { probeCaptureGet, probeReset, probeStatus, probeWaitHit } from "@tools-feature-probe";
import {
  buildLineKey,
...
test("[IT][java-agent][probe] protected createPost route hits strict controller line", async () => {
...
test("[IT][java-agent][probe] declaration-line strict key fails closed as invalid_line_target", async () => {
  if (!runtime) throw new Error("post-app runtime was not started");
...
test("[IT][java-agent][probe] lambda-mapped source line is strict-probeable via owning method key", async () => {
  if (!runtime) throw new Error("post-app runtime was not started");
...
test("[IT][java-agent][probe] actuate endpoint rejects unauthorized requests when token is configured", async () => {
  const securedRuntime = await startPostAppWithAgent({
    actuateAuthToken: "fixture-actuate-secret",
  });
  try {
...
test("[IT][java-agent][probe] include supports additional package scope from sibling module", async () => {
...
test("[IT][java-agent][probe] include basepaths instrument an exact class outside include glob", async () => {
