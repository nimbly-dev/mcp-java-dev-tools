...
/** Replaces the generated superset schema with the action-correlated public schema. */
public final class FailureAnalysisMcpSchemaPostProcessor implements BeanPostProcessor {

    private static final String TOOL_NAME = "failure_analysis";
    ...
    private Map<String, Object> parseSchema() {
        try {
            return objectMapper.readValue(
                    FailureAnalysisMcpSchema.publicInputSchema(objectMapper), new TypeReference<>() { });
        } catch (JsonProcessingException exception) {
            throw new IllegalStateException("failure_analysis schema must be valid JSON", exception);
        }
    }
}
