...
test("[IT][execution_orchestration][execute] execution_orchestration preserves probe.hit across ordered authenticated suite plans on social-platform fixture", async () => {
  const tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "mcp-auth-ordered-suite-probe-line-hit-it-"),
  );
  ...
  try {
    ...
    assert.equal(typeof createExecution.steps[0]?.assertions[1]?.actual, "undefined");
    ...
    assert.equal(typeof feedExecution.steps[0]?.assertions[1]?.actual, "undefined");
  } finally {
    await mcp?.close();
    await runtime?.stop();
