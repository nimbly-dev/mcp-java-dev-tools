...
test("[IT][execution_orchestration][execute] local cross-service regression waits on downstream processing and verifies indexed_count=500 via SQL", async () => {
  const tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "mcp-event-cross-service-watcher-sql-it-"),
  );
  ...
  try {
    ...
    if (
      process.env.KEEP_EVENT_CROSS_SERVICE_WATCHER_SQL_TMP !== "1" &&
      fssync.existsSync(tmpRoot)
    ) {
