      ...
      steps: [
        {
          ...
          expect: [
            { id: "e1", actualPath: "response.statusCode", operator: "numeric_gte", expected: 200 },
          ],
        },
      ],
    },
...
function writePerformancePlanContract(root: string, projectName: string, planName: string): void {
  writeJson(
    path.join(root, ".mcpjvm", projectName, "plans", "performance", planName, "metadata.json"),
    {
      suiteType: "performance",
  ...
  writeJson(
    path.join(root, ".mcpjvm", projectName, "plans", "performance", planName, "contract.json"),
    {
      entrypoints: [
        {
          transport: {
...
function writePerformanceJmeterPlanContract(
  root: string,
  projectName: string,
  planName: string,
): void {
...
function writePerformanceProjectWithExplicitProbeBaseUrl(root: string): void {
  ...
  writeJson(
    path.join(
      ...
      "plans",
      "performance",
      "type-performance-review-by-course",
      "metadata.json",
    ),
    {
      suiteType: "performance",
...
test("[UT][execution-profile-export][export_execution_profile] executionProfileExportDomain resolves executionProfile and creates a fresh sh export", async () => {
  ...
  try {
    ...
    assert.match(
      String(out.structuredContent.exportId ?? ""),
      /^20\d{6}-\d{6}-regression-test-run$/,
    );
    assert.match(
...
test("[UT][execution-profile-export][export_execution_profile] executionProfileExportDomain uses real suite artifact time for when-based source selection", async () => {
  ...
  try {
      ...
      planRuns: [
        {
          ...
          planName: "gateway-route-smoke-spec",
          status: "blocked",
          blockedReasonCode: "probe_gate_failed",
        },
      ],
      completedPlanCount: 1,
    });
    fs.utimesSync(
      olderSuitePath,
      new Date("2026-06-13T04:39:27.000Z"),
      new Date("2026-06-13T04:39:27.000Z"),
    );
    ...
    fs.utimesSync(
      newerSuitePath,
      new Date("2026-06-13T04:51:12.000Z"),
      new Date("2026-06-13T04:51:12.000Z"),
    );
...
test("[UT][execution-profile-export][export_execution_profile] executionProfileExportDomain resolves project ambiguity when projectName is explicitly provided", async () => {
  ...
  try {
    ...
    writeProjectWithName(
      root,
      "test-project-performance",
      "test-performance-contract-run",
...
test("[UT][execution-profile-export][export_execution_profile] executionProfileExportDomain exports performance replay package for sh mode", async () => {
  ...
  try {
    ...
    assert.match(
      String(out.structuredContent.exportId ?? ""),
      /^20\d{6}-\d{6}-type-performance-hello-suite$/,
    );
...
test("[UT][execution-profile-export][export_execution_profile] executionProfileExportDomain exports performance replay package for ps1 mode", async () => {
  ...
  try {
    ...
    assert.match(
      String(out.structuredContent.output?.scriptPathAbs ?? ""),
      /run-performance-profile\.ps1$/,
    );
    assert.match(
      String(out.structuredContent.output?.readmePathAbs ?? ""),
      /README\.performance\.ps1\.md$/,
    );
...
test("[UT][execution-profile-export][export_execution_profile] executionProfileExportDomain exports JMeter-compatible performance replay package for sh mode", async () => {
  ...
  try {
    ...
    const jmxPathAbs = path.join(
      exportDirAbs,
      "artifacts",
      "jmeter",
      "type-performance-jmeter.workload.jmeter.jmx",
    );
...
test("[UT][execution-profile-export][export_execution_profile] exported performance replay runner executes JMeter-backed package end-to-end", async () => {
  ...
  try {
    ...
    const planResult = readJson(
      path.join(runDirAbs, "type-performance-jmeter.execution.result.json"),
    );
...
test("[UT][execution-profile-export][export_execution_profile] executionProfileExportDomain exports postman collection when scripts are JS-compatible", async () => {
  ...
  try {
    ...
    assert.match(
      String(out.structuredContent.output?.collectionPathAbs ?? ""),
      /collection\.postman\.json$/,
    );
    assert.match(
      String(out.structuredContent.output?.environmentPathAbs ?? ""),
    ...
    assert.match(
      String(collection2.info.name ?? ""),
      /^execution-profile:regression-test-run:\d{8}-\d{6}-regression-test-run$/,
    );
    assert.match(
      String(collection.info.name ?? ""),
      /^execution-profile:regression-test-run:\d{8}-\d{6}-regression-test-run$/,
    );
    assert.deepEqual(
      { ...collection2, info: { ...collection2.info, name: "<normalized>" } },
      { ...collection, info: { ...collection.info, name: "<normalized>" } },
    );
      ...
      steps: [
        {
          ...
          targetRef: 0,
          protocol: "http",
          transport: { http: { method: "GET", url: "http://127.0.0.1:8080/a" } },
          expect: [
            { id: "e1", actualPath: "response.statusCode", operator: "numeric_gte", expected: 200 },
          ],
        },
      ...
      steps: [
        {
          ...
          targetRef: 0,
          protocol: "http",
          transport: { http: { method: "GET", url: "http://127.0.0.1:8080/b" } },
          expect: [
            { id: "e2", actualPath: "response.statusCode", operator: "numeric_gte", expected: 200 },
          ],
        },
      ],
    });
...
test("[UT][execution-profile-export][export_execution_profile] executionProfileExportDomain supports derived required prerequisite via postman extract before downstream use", async () => {
  ...
  try {
    ...
    writePlanArtifact(root, "gateway-route-smoke-spec", {
      targets: [{ type: "class_method", selectors: { fqcn: "x.A", method: "m" } }],
      prerequisites: [
        { key: "courseId", required: true, secret: false, provisioning: "user_input" },
      ],
      steps: [
        {
          order: 1,
        ...
        },
        {
          ...
          protocol: "http",
          transport: {
            http: { method: "GET", url: "http://127.0.0.1:8080/api/course/{{courseId}}" },
          },
          expect: [
            { id: "e2", actualPath: "response.statusCode", operator: "numeric_gte", expected: 200 },
          ],
...
test("[UT][execution-profile-export][export_execution_profile] executionProfileExportDomain executes non-JS prerequisite script before postman export", async () => {
  ...
  try {
    ...
    fs.mkdirSync(path.join(root, ".mcpjvm", "test-project", "scripts"), { recursive: true });
    fs.writeFileSync(
      path.join(root, ".mcpjvm", "test-project", "scripts", "setup.ps1"),
      "Write-Output 'x'\n",
      "utf8",
    );
