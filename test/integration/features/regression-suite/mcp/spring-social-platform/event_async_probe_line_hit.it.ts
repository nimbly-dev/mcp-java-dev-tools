...
test("[IT][execution_orchestration][execute] execution_orchestration preserves probe.hit for authenticated async event trigger on controller and listener targets", async () => {
  ...
  try {
        ...
        targets: [
          {
            type: "class_method",
            selectors: {
              fqcn: args.strictProbeKey.startsWith(eventControllerFqcn)
                ? eventControllerFqcn
                : eventListenerFqcn,
              method: args.strictProbeKey.startsWith(eventControllerFqcn)
                ? "triggerIndex"
                : "receiveEvent",
    ...
    assert.equal(typeof controllerExecution.steps[0]?.assertions[2]?.actual, "undefined");
    ...
    assert.equal(typeof listenerExecution.steps[0]?.assertions[2]?.actual, "undefined");
  } finally {
    await mcp?.close();
    await runtime?.stop();
