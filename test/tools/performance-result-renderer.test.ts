...
test("result renderer blocks when an available correlation Artifact is missing", async () => {
...
test("result renderer emits an unavailable correlation placeholder without a method table", async () => {
  await withRunDir(async (runDir) => {
    await writeBaseArtifacts(runDir, {
      status: "fail",
...
test("result renderer renders the specified method table for valid correlation evidence", async () => {
  await withRunDir(async (runDir) => {
    await writeBaseArtifacts(runDir, {
      status: "pass",
      metrics: { durationMs: 15000, errorRatePct: 0, throughputPerSec: 10, p95LatencyMs: 20 },
