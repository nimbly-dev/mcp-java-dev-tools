...
async function seedExpiredState(root: string, withSuiteLink = true): Promise<void> {
  ...
  db.prepare(
    ...
    JSON.stringify({
      runtimeSuite: { executionProfile: "profile-1", executionPolicy: "stop_on_fail" },
    }),
  );
  const suitePk = db
  ...
  if (withSuiteLink)
    links.push([
      ...
      ".mcpjvm/alpha/suite-runs/suite-1/execution_orchestration.result.json",
    ]);
  ...
  if (withSuiteLink)
    writeJson(
      path.join(root, ".mcpjvm/alpha/suite-runs/suite-1/execution_orchestration.result.json"),
      {
        status: "in_progress",
...
test("[UT][artifact-management][run_state_expired_reconcile][ok] reconcileExpiredActiveState terminalizes one overdue Watcher without provider execution", async () => {
  ...
  try {
    ...
    assert.equal(
      db.prepare("SELECT status FROM watcher_runs WHERE watcher_name = 'watcher-1'").get().status,
      "blocked_runtime",
    );
    assert.equal(
      db
        .prepare(
          "SELECT outcome, reason_code, continuation_json FROM watcher_runs WHERE watcher_name = 'watcher-1'",
        )
        .get().outcome,
      "timed_out",
    );
    assert.equal(
      db
        .prepare(
          "SELECT owner_id, lease_expires_at_epoch_ms, status FROM suite_runs WHERE suite_run_id = 'suite-1'",
        )
        .get().owner_id,
      null,
    );
    ...
    const execution = JSON.parse(
      fs.readFileSync(
        path.join(root, ".mcpjvm/alpha/plans/regression/p1/runs/r1/execution.result.json"),
        "utf8",
      ),
    );
