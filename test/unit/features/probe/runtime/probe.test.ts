...
const {
  ...
  probeProfiler,
  createProbeDomain,
} = require("@tools-feature-probe");
const { LAST_RESET_EPOCH_BY_KEY } = require("@tools-feature-probe");
...
test("[UT][probe][probe] probe_get_status supports 0.1.0 nested envelope", async () => {
  await withMockedFetch(
    ...
    async () => {
      ...
      assert.equal(
        out.structuredContent.response.json.capturePreview.executionStartedAtEpoch,
        5500,
      );
      ...
      assert.equal(
        out.structuredContent.response.json.capturePreview.threadAllocatedBytesDelta,
        2048,
      );
...
test("[UT][probe][probe] probe_wait_for_hit timeout_no_inline_hit returns line-not-executed guidance", async () => {
  ...
  await withMockedFetch(
    ...
    async () => {
      ...
      assert.equal(
        parsed.nextAction,
        "verify_trigger_path_or_branch_then_rerun_probe_wait_for_hit",
      );
  ...
  try {
    ...
    const domain = createProbeDomain({
      ...
      getProbeRegistry: () => ({
        ...
        probesById: new Map([
          [
            "producer-service",
            { id: "producer-service", baseUrl: "http://127.0.0.1:9191", include: [], exclude: [] },
          ],
          [
            "consumer-service",
            { id: "consumer-service", baseUrl: "http://127.0.0.1:9192", include: [], exclude: [] },
          ],
        ]),
      }),
    });
    ...
    await withMockedFetch(
      async (input, init) => {
        ...
        if (
          url === `http://127.0.0.1:9192/__probe/status?key=${encodeURIComponent(key)}` &&
          method === "GET"
        ) {
          return jsonResponse(200, {
            key,
  ...
  const domain = createProbeDomain({
    ...
    getProbeRegistry: () => ({
      ...
      probesById: new Map([
        ...
        [
          "auth-service",
          { id: "auth-service", baseUrl: "http://127.0.0.1:9191", include: [], exclude: [] },
        ],
      ]),
    }),
  });
