...
async function callTool(
  ...
  if (typeof options?.timeoutMs === "number") {
    return (await mcp.client.callTool(request, undefined, {
...
test("[IT][execution_orchestration][execute] execution_orchestration honors extended strict probe wait budget for long-running listener processing",
  { timeout: 240_000 },
  async () => {
    const tmpRoot = await fs.mkdtemp(
      path.join(os.tmpdir(), "mcp-event-long-listener-probe-late-hit-it-"),
    );
