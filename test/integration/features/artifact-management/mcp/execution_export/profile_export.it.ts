...
async function writeProjectArtifacts(args: {
  ...
  const profiles = args.executionProfiles.map((executionProfile, index) => ({
    ...
    plans: [
      { order: 1, planName: args.planNames?.[index] ?? `plan-${index + 1}`, onFail: "inherit" },
    ],
  }));
...
test("[IT][artifact_management][execution_export] execution_profile_export fails closed when project context is ambiguous", async () => {
  const tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "mcp-execution-profile-export-ambig-project-it-"),
  );
...
test("[IT][artifact_management][execution_export] execution_profile_export honors explicit projectName in multi-project workspace", async () => {
  const tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "mcp-execution-profile-export-project-selector-it-"),
  );
  const workspaceRootAbs = path.join(tmpRoot, "workspace");
  const probeConfigAbs = path.join(workspaceRootAbs, ".mcpjvm", "probe-config.json");
