...
test("[IT][artifact_management][run_result] artifact_management covers probe_config/regression_plan/run_result/execution_export flows", async () => {
  ...
  await fs.writeFile(
    path.join(exportsRoot, "run-execution-profile.sh"),
    "#!/usr/bin/env sh\n",
    "utf8",
  );
  ...
  try {
    ...
    assert.equal(
      (planReadSummary.structuredContent?.summary as { stepCount?: unknown })?.stepCount,
      3,
    );
    ...
    assert.equal(
      typeof (planReadDetail.structuredContent?.artifact as { contract?: unknown })?.contract,
      "object",
    );
    ...
    assert.equal(
      (planReadWindowed.structuredContent?.summary as { prerequisiteCount?: unknown })
        ?.prerequisiteCount,
      4,
    );
    assert.equal(Array.isArray(planReadWindowed.structuredContent?.targets), true);
    assert.equal(
      (planReadWindowed.structuredContent?.prerequisites as { offset?: unknown })?.offset,
      1,
    );
    assert.equal(
      (planReadWindowed.structuredContent?.prerequisites as { returned?: unknown })?.returned,
      2,
    );
    assert.equal(
      ((planReadWindowed.structuredContent?.prerequisites as { items?: Array<{ key?: unknown }> })
        ?.items ?? [])[0]?.key,
      "ctx-2",
    );
    assert.equal((planReadWindowed.structuredContent?.steps as { offset?: unknown })?.offset, 1);
    assert.equal(
      (planReadWindowed.structuredContent?.steps as { returned?: unknown })?.returned,
      1,
    );
    assert.equal(
      ((planReadWindowed.structuredContent?.steps as { items?: Array<{ id?: unknown }> })?.items ??
        [])[0]?.id,
      "health_check_2",
    );
    ...
    assert.equal(
      (runReadSummary.structuredContent?.summary as { stepCount?: unknown })?.stepCount,
      1,
    );
...
test("[IT][artifact_management][run_result] artifact_management run_result read honors explicit projectName in multi-project workspace", async () => {
  const tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "mcp-artifact-management-run-read-project-it-"),
  );
  const workspaceRootAbs = path.join(tmpRoot, "workspace");
...
test("[IT][artifact_management][run_result] artifact_management run_result watcher queries are bounded and deterministic", async () => {
  ...
  try {
    ...
    assert.equal(
      (filtered.structuredContent?.summary as { triggerStatus?: unknown })?.triggerStatus,
      "pass",
    );
    assert.equal(
      (filtered.structuredContent?.summary as { watcherStatus?: unknown })?.watcherStatus,
      "blocked",
    );
    assert.equal((filtered.structuredContent?.watchers as { total?: unknown })?.total, 1);
    assert.equal(
      ((filtered.structuredContent?.watchers as { items?: Array<{ id?: unknown }> })?.items ??
        [])[0]?.id,
      "analytics-rollup",
    );
    assert.equal((filtered.structuredContent?.watcherEvidence as { total?: unknown })?.total, 1);
    assert.equal(
      ((filtered.structuredContent?.watcherEvidence as { items?: Array<{ id?: unknown }> })
        ?.items ?? [])[0]?.id,
      "analytics-rollup",
    );
  } finally {
