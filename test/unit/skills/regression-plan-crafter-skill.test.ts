...
test("[UT][skills][regression_plan_crafter_skill] regression plan crafter skill is portable with bundled references and templates", () => {
  ...
  assert.ok(
    fs.existsSync(path.join(skillDir, "references", "templates", "metadata.template.json")),
  );
  assert.ok(
    fs.existsSync(path.join(skillDir, "references", "templates", "contract.template.json")),
  );
...
test("[UT][skills][regression_plan_crafter_skill] bundled crafter templates can bootstrap a plan package inside test/.tmp workspace", () => {
  ...
  try {
    ...
    contractTemplate.targets[0].selectors.fqcn =
      "com.example.social.post.app.controller.PostController";
    ...
    fs.writeFileSync(
      path.join(targetDir, "contract.json"),
      `${JSON.stringify(contractTemplate, null, 2)}\n`,
    );
    fs.writeFileSync(
      path.join(targetDir, "plan.md"),
      planTemplate.replaceAll("<regression_name>", regressionName),
    );
...
test("[UT][skills][regression_plan_crafter_skill] crafted template marks secret prerequisites without persisted defaults", () => {
  ...
  const secretKeysWithDefaults = contractTemplate.prerequisites
    ...
    .filter(
      (entry: { key: string; secret: boolean; default?: unknown }) =>
...
test("[UT][skills][regression_plan_crafter_skill] regression suite skill remains execution-focused and result skill is available separately", () => {
  ...
  assert.match(
    suiteText,
    /Always use bounded or windowed reads for Artifact inspection, logs, and generated scripts/i,
  );
  ...
  assert.match(
    suiteText,
    /load each ordered plan through `artifact_management` .* explicit pagination/i,
  );
  assert.match(
    suiteText,
    /do not read `contract\.json`, `metadata\.json`, or `plan\.md` directly/i,
  );
  assert.match(
    suiteText,
    /the orchestrator should load runtime suite plan inputs through these paged `artifact_management` calls/i,
  );
  assert.match(
    suiteText,
    /`artifact_management` is the maintained read path for execution profile lookup and regression plan loading/i,
  );
  ...
  assert.match(
    suiteText,
    /Do not summarize a runtime-suite `execution_profile` run as completed until the terminal orchestration status is returned/i,
  );
  assert.match(
    suiteText,
    /Do not treat a caller\/tool-boundary timeout as the primary execution result/i,
  );
  assert.match(
    suiteText,
    /Do not reuse a stale suite artifact as the result of a fresh execution request/i,
  );
