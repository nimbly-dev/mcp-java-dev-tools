...
function loadBundledSkill(skillName: string) {
  ...
  const template = readUtf8(
    path.join(skillDir, "references", "templates", "execution_profile_export.md"),
  );
...
test("[UT][skills][execution_profile_export_skill] Regression Export Skill Workflow is portable with bundled references and templates", () => {
  ...
  assert.ok(
    fs.existsSync(
      path.join(loaded.skillDir, "references", "templates", "execution_profile_export.md"),
    ),
  );
});
...
test("[UT][skills][execution_profile_export_skill] Performance Export Skill Workflow remains single-mode and deterministic", () => {
  ...
  assert.match(
    loaded.templatesIndex,
    /Default template id: `performance_execution_profile_export`/,
  );
