...
| `executionResult.externalVerification[]` | Per-verification normalized downstream data-validity result with deterministic fail-closed reason codes, assertion/extract diagnostics, and a compact persisted HTTP response summary (`statusCode`, `durationMs`, `bodyFormat`, `bodyBytes`, `hasBodyJson`, optional `headerNames`). Raw response bodies, raw header values, assertion `actual` values, extract `value` fields, and `extractedContext` values are not persisted. | `mcp-java-dev-tools-regression-suite` | false | `[{"id":"verify_reindex_task_status","providerType":"http","status":"pass","response":{"statusCode":200,"bodyFormat":"json","bodyBytes":18,"hasBodyJson":true}}]` |
| `evidence.externalVerificationExecutions[]` | Persisted external verification execution rows using the same compact response summary shape as `executionResult.externalVerification[]` for Artifact review/debugging without storing raw third-party payloads or extracted/asserted values. | `mcp-java-dev-tools-regression-suite` | false | `[{"id":"verify_reindex_task_status","providerType":"http","status":"pass","response":{"statusCode":200,"bodyFormat":"json","bodyBytes":18,"hasBodyJson":true}}]` |
