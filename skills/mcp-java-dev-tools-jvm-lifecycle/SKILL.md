...
2. Treat every returned descriptor as unverified. Never infer a target from PID order or a display name.
3. If more than one candidate could be the requested target, require the operator to provide one exact PID.
4. Before mutation, obtain explicit confirmation for that PID and state the selected Probe host/port.
5. Call `jvm_lifecycle` with `action="attach"`, the exact string PID, `confirm=true`, and the selected loopback Probe host/port. Supply `include`/`exclude` only when the operator provides the intended instrumentation scope.
...
1. Require an exact PID and explicit confirmation before calling `jvm_lifecycle` with `action="deactivate"` and `confirm=true`.
...
- Never expose full JVM command lines, environment values, system-property values, or secrets in the summary.
