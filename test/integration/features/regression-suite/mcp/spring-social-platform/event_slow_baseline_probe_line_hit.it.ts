...
test("[IT][execution_orchestration][execute] execution_orchestration preserves probe.hit when manual strict verification resolves via already_hit_since_inline_start", async () => {
  const tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "mcp-event-slow-baseline-probe-line-hit-it-"),
  );
  ...
  try {
          ...
          executionProfiles: [
            {
              ...
              plans: [
                { order: 1, planName: "slow-baseline-producer-trigger-plan", onFail: "inherit" },
              ],
            },
          ],
        },
    ...
    const manualResetResponse = manualReset.structuredContent?.response as
    ...
    const manualWaitRequest = manualWait.structuredContent?.request as
      | Record<string, unknown>
      | undefined;
    const manualWaitResult = manualWait.structuredContent?.result as
      | Record<string, unknown>
      | undefined;
    assert.equal(
      manualWaitResult?.hit,
      true,
      JSON.stringify(manualWait.structuredContent, null, 2),
    );
    ...
    assert.ok(
      Number(manualWaitRequest?.triggerLeadMs) >= 1_500,
      JSON.stringify(manualWait.structuredContent, null, 2),
    );
