...
import { startEventCrossServiceRegressionSuiteFixture } from "@test/support/spring/social-platform/regression_suite.fixture";
...
async function runCase(name: string, options: CaseOptions = {}) {
  const fixture = await startEventCrossServiceRegressionSuiteFixture({
    ...
    ...(name === "missing-consumer"
      ? { producerExtraJavaArgs: ["-Dfixture.consumer.forward-enabled=false"] }
      : {}),
  });
  try {
    ...
    if (options.expectedReason) {
      assert.equal(
        correlation.status,
        "fail_closed",
        JSON.stringify({ output, execution, correlation }, null, 2),
      );
      ...
      assert.ok(
        timeline.some(
          (event) =>
            event.eventType === "runtime_line_hit" && event.probeId === fixture.consumerProbeId,
        ),
      );
    }
  } finally {
...
test("[IT][execution_orchestration][execute] generic JDK async handoff propagates the HTTP eventId through the consumer event object", async () => {
  await runCase("happy");
});

test("[IT][execution_orchestration][execute] wrong consumer eventId fails closed as correlation_key_not_observed", async () => {
  await runCase("wrong-key", {
    notes: "fixture-consumer-event-id:evt-wrong",
    expectedReason: "correlation_key_not_observed",
  });
});

test("[IT][execution_orchestration][execute] missing consumer event fails closed as missing_expected_flow_event", async () => {
  await runCase("missing-consumer", { expectedReason: "missing_expected_flow_event" });
});

test("[IT][execution_orchestration][execute] runtime evidence outside the configured Probe scope fails closed", async () => {
  await runCase("probe-scope", {
    correlationProbeIds: ["event-producer-app"],
    expectedReason: "correlation_probe_scope_mismatch",
  });
});

test("[IT][execution_orchestration][execute] delayed async processing fails closed after the correlation window", async () => {
  await runCase("window", {
    notes: "fixture-listener-delay-ms:1000",
    maxWindowMs: 100,
    expectedReason: "window_exceeded",
  });
});
