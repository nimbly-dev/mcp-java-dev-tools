...
function renderJsonHelperSection(): string[] {
  return [
    ...
    "path_raw = sys.argv[2]",
    "segments = []",
    "for part in (path_raw.split('.') if path_raw else []):",
    "    if not part:",
    "        continue",
    "    pos = 0",
    "    while pos < len(part):",
    "        match = re.match(r'([^[\\]]+)|\\[(\\d+)\\]', part[pos:])",
    "        if not match:",
    "            print('')",
    "            sys.exit(0)",
    "        key = match.group(1)",
    ...
    "    sys.exit(0)",
    "for seg in segments:",
    "    if isinstance(seg, int) and isinstance(node, list) and 0 <= seg < len(node):",
    "        node = node[seg]",
    ...
    "    node -e \"const raw = process.argv[1]; const pathRaw = process.argv[2] || ''; const segments = []; for (const part of pathRaw.split('.')) { if (!part) continue; let pos = 0; while (pos < part.length) { const match = /^([^[\\]]+)|^\\[(\\d+)\\]/.exec(part.slice(pos)); if (!match) { process.stdout.write(''); process.exit(0); } if (typeof match[1] === 'string') segments.push(match[1]); else segments.push(Number(match[2])); pos += match[0].length; } } let node; try { node = JSON.parse(raw); } catch { process.stdout.write(''); process.exit(0); } for (const seg of segments) { if (Number.isInteger(seg) && Array.isArray(node) && seg >= 0 && seg < node.length) node = node[seg]; else if (node && typeof node === 'object' && !Array.isArray(node) && Object.prototype.hasOwnProperty.call(node, seg)) node = node[seg]; else { process.stdout.write(''); process.exit(0); } } process.stdout.write(node == null ? '' : String(node));\" \"$json\" \"$path\"",
    "    return",
    "  fi",
