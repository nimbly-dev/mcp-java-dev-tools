...
test("[IT][execution_orchestration][execute] execution_orchestration execute uses runtime suite path and does not create exports", async () => {
...
test("[IT][execution_orchestration][execute] execution_orchestration execute does not fail with project_artifact_ambiguous when projectName is explicit in multi-project workspace", async () => {
...
test("[IT][execution_orchestration][execute] execution_orchestration accepts compatible {{key}} transport placeholder syntax and reaches plan execution", async () => {
  const tmpRoot = await fs.mkdtemp(
...
test("[IT][execution_orchestration][execute] execution_orchestration resolves project contextBindings from env-backed workspace mappings", async () => {
  const tmpRoot = await fs.mkdtemp(
...
test("[IT][execution_orchestration][execute] execution_orchestration passes transport-failure contract-matched assertions and persists clean step artifacts", async () => {
  const tmpRoot = await fs.mkdtemp(
...
test("[IT][execution_orchestration][execute] execution_orchestration keeps optional-only transport-failure step failures out of overall run status", async () => {
  const tmpRoot = await fs.mkdtemp(
...
test("[IT][execution_orchestration][execute] execution_orchestration supports array index notation in response body assertions", async () => {
  const tmpRoot = await fs.mkdtemp(
...
test("[IT][execution_orchestration][execute] execution_orchestration surfaces blockedReasonMeta for http_payload_invalid when apiBaseUrl is missing for pathTemplate synthesis", async () => {
  const tmpRoot = await fs.mkdtemp(
...
test("[IT][execution_orchestration][execute] execution_orchestration composes baseUrl prerequisite with transport path when url is absent", async () => {
  const tmpRoot = await fs.mkdtemp(
...
test("[IT][execution_orchestration][execute] execution_orchestration resolves correlation when json_path source uses response.body.id", async () => {
  const tmpRoot = await fs.mkdtemp(
...
test("[IT][execution_orchestration][execute] execution_orchestration records unresolved optional extract without blocking the run", async () => {
  const tmpRoot = await fs.mkdtemp(
...
test("[IT][execution_orchestration][execute] execution_orchestration continue_on_fail stops after suite-level env blocker and creates no runs", async () => {
  const tmpRoot = await fs.mkdtemp(
