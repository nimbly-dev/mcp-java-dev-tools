...
test("[UT][execution-orchestration][execute_execution_orchestration] executionOrchestrationDomain resumes persisted suite progress by suiteRunId without rerunning completed plans", async () => {
  ...
  try {
    ...
    assert.equal(
      first.structuredContent.status,
      "in_progress",
...
test("[UT][execution-orchestration][execute_execution_orchestration] executionOrchestrationDomain uses SQLite-only suite state after cutover", async () => {
  ...
  try {
    ...
    const first = await executionOrchestrationDomain({
      workspaceRootAbs: root,
      payload: { projectName, executionProfile: "post-cutover-suite", maxPlansPerCall: 1 },
    });
    ...
    const afterDeletion = await executionOrchestrationDomain({
      workspaceRootAbs: root,
      payload: {
        projectName,
        executionProfile: "post-cutover-suite",
    ...
    const store = new (require("node:sqlite").DatabaseSync)(
      path.join(root, ".mcpjvm", projectName, "run-state.sqlite"),
    );
    try {
      assert.equal(
        store
          .prepare(
            "SELECT COUNT(*) AS count FROM artifacts WHERE artifact_kind = 'execution_orchestration'",
          )
          .get().count,
        0,
      );
...
test("[UT][execution-orchestration][execute_execution_orchestration] executionOrchestrationDomain auto-resumes fresh calls without explicit maxPlansPerCall", async () => {
  ...
  try {
    ...
    const planRuns = Array.isArray(out.structuredContent.planRuns)
      ? out.structuredContent.planRuns
      : [];
