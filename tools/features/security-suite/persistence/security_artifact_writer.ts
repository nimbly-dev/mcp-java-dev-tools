...
function sanitizeSecurityDiagnostic(value: string): string {
  ...
  sanitized = sanitized.replace(
    /([?&](?:token|password|secret|api[_-]?key|credential)=)[^&#\s]+/gi,
    `$1${REDACTED_VALUE}`,
  );
  sanitized = sanitized.replace(
...
function sanitizeSecurityFinding(finding: SecurityFinding): SecurityFinding {
  return {
    ...
    ...(finding.attackProfileRef
      ? { attackProfileRef: sanitizeSecurityId(finding.attackProfileRef) }
      : {}),
  };
}
...
function sanitizeSecurityEvidence(evidence: SecurityEvidenceReference): SecurityEvidenceReference {
  return {
    ...
    ...(evidence.artifactPath
      ? { artifactPath: sanitizeSecurityDiagnostic(evidence.artifactPath) }
      : {}),
    redacted: true,
  };
}
