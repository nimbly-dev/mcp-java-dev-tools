...
test("[IT][route_synthesis][create_recipe] uses built-in fallback template profile when no project fixture profile is provided", async () => {
...
test("[IT][route_synthesis][create_recipe] applies project fixture profile overrides for deterministic request templates", async () => {
  if (!mcp) throw new Error("mcp not initialized");
  await writeTemplateProfile(
    ["sample.path.param.postId=101", 'sample.body.default={"content":"fixture-body"}'].join("\n"),
  );
