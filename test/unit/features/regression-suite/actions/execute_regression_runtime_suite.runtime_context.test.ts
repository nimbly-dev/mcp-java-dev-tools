...
test("[UT][regression-suite][execute_regression_runtime_suite_runtime_context] executeRegressionRuntimeSuite resolves project contextBindings into transport context and redacts env-sourced values from persisted context", async () => {
  ...
  try {
    ...
    const context = JSON.parse(
      fs.readFileSync(path.join(runsRoot, runIds[0], "context.resolved.json"), "utf8"),
    );
          ...
          executionProfiles: [
            {
              ...
              executionPolicy: "stop_on_fail",
              plans: [
                { order: 1, planName, providedContext: { "auth.bearer": "runtime-secret-token" } },
              ],
            },
          ],
        },
              ...
              healthChecks: [
                {
                  id: "http-ready",
                  type: "http",
                  url: `http://127.0.0.1:${liveHealthPort}/health`,
                  required: true,
                },
              ],
            },
