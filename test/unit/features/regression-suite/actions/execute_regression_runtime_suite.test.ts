  ...
  try {
    ...
    const second = await executeRegressionRuntimeSuite({
      ...
      mcpInvoke: async ({ input }: { toolName: string; input: Record<string, unknown> }) => {
        ...
        return {
          structuredContent: { status: "pass", statusCode: 200, durationMs: 1, bodyPreview: "{}" },
        };
      },
    });
  ...
  try {
    ...
    const out = await executeRegressionRuntimeSuite({
      ...
      mcpInvoke: async ({
        ...
        return {
          structuredContent: { status: "pass", statusCode: 200, durationMs: 7, bodyPreview: "{}" },
        };
      },
    });
        ...
        if (String(request.method) === "POST") {
          return {
            structuredContent: {
              ...
              statusCode: 202,
              durationMs: 1,
              body: '{"eventId":"evt-700"}',
              bodyPreview: '{"eventId":"evt-700"}',
            },
          };
        }
        await new Promise((resolve) => setTimeout(resolve, 15));
      ...
      targets: [
        {
          type: "class_method",
          selectors: {
            fqcn: "org.example.TasksController",
            method: "submit",
            sourceRoot: "src/main/java",
          },
        },
        ...
        if (String(request.method) === "POST") {
          ...
          return {
            structuredContent: {
              ...
              statusCode: 202,
              durationMs: 1,
              body: '{"taskId":"task-700"}',
              bodyPreview: '{"taskId":"task-700"}',
            },
          };
        }
...
test("[UT][regression-suite][execute_regression_runtime_suite] executeRegressionRuntimeSuite surfaces blockedReasonMeta when http payload is invalid from missing synthesized url", async () => {
  ...
  try {
    ...
    const out = await executeRegressionRuntimeSuite({
      ...
      mcpInvoke: async () => {
        throw new Error(
          "mcpInvoke should not be called when http payload is invalid before network execution",
        );
      },
    });
    ...
    assert.equal(
      out.planRuns[0].blockedReasonMeta?.cause,
