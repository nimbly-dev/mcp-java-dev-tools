...
test("[IT][execution_orchestration][execute] cross-service event suite preserves probe verification and correlation artifacts end-to-end with independent Probe confirmation", async () => {
  ...
  try {
    ...
    const producerCorrelationPolicy = (producerEvidence.correlationPolicy ?? {}) as Record<
      ...
      unknown
    >;
    const consumerCorrelationPolicy = (consumerEvidence.correlationPolicy ?? {}) as Record<
      string,
      unknown
    >;
    ...
    >;
    const consumerEvents = (consumerEvidence.correlationEvents ?? []) as Array<
      Record<string, unknown>
    >;
    ...
    const producerProbeJson = ((
      producerStatus.structuredContent?.response as Record<string, unknown>
    )?.json ?? {}) as Record<string, unknown>;
    const consumerProbeJson = ((
      consumerStatus.structuredContent?.response as Record<string, unknown>
    )?.json ?? {}) as Record<string, unknown>;
    ...
    const producerCapturePreview = (producerProbeJson.capturePreview ?? {}) as Record<
      string,
      unknown
    >;
    const consumerCapturePreview = (consumerProbeJson.capturePreview ?? {}) as Record<
      string,
      unknown
    >;
