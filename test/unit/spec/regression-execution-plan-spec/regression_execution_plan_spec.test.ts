...
test("[UT][regression-execution-plan-spec][regression_execution_plan_spec] applyStepExtract supports array index notation in extract paths", () => {
  ...
  const next = applyStepExtract(
    output,
    [{ from: "response.bodyJson.names[0].value", as: "primaryName" }],
    initial,
  );
  assert.equal(next.tenantId, "tenant-social-001");
    ...
    steps: [
      {
        ...
        expect: [
          { id: "accepted", actualPath: "status", operator: "outcome_status", expected: "pass" },
        ],
      },
    ],
  });
