...
function validateExpectationEntries(args: {
  ...
  if (!Array.isArray(args.expectations) || args.expectations.length === 0) {
    return {
      ...
      requiredUserAction: [`Add deterministic ${contractPath} entries for ${args.ownerType} '${args.ownerId}'.`],
    };
  }
  ...
  for (const raw of args.expectations) {
    ...
    if (!isRecord(expectation)) {
      return {
        ...
        requiredUserAction: [`Ensure all expectations for ${args.ownerType} '${args.ownerId}' are objects.`],
      };
    }
    ...
    if (!hasNonBlank(expectation.id)) {
      return {
        ok: false,
        reasonCode: invalidReasonCode,
        requiredUserAction: [`Set non-empty expectation id for ${args.ownerType} '${args.ownerId}'.`],
      };
    }
  ...
  for (const step of steps) {
    ...
    if (!result.ok) {
      return {
        ok: false,
        reasonCode:
          result.reasonCode === "step_expectations_missing" ? "step_expectations_missing" : "step_expectation_invalid",
