...
const creatorPulseControllerFqcn = "com.example.social.post.app.controller.CreatorPulseController";
...
test("[IT][route_synthesis][create_recipe] bytecode annotation fallback resolves interface mapping without source bodies", async () => {
  ...
  assert.equal(
    recipe.structuredContent.attemptedStrategies.includes(bytecodeFallbackStrategy),
    true,
  );
  assert.equal(recipe.structuredContent.evidence.includes(bytecodeMappingEvidence), true);
});

test("[IT][route_synthesis][create_recipe] bytecode annotation fallback resolves interface->abstract->concrete chain", async () => {
  const recipe = await createRecipe({
