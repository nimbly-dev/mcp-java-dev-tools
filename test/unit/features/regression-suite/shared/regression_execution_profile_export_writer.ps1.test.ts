...
const { exportExecutionProfilePs1 } = require("@tools-export-execution-profile");
          ...
          runtimeContexts: [
            {
              ...
              mode: "terminal",
              startups: [
                { name: "gateway-service", command: "java", args: ["-jar", "gateway.jar"] },
              ],
            },
          ],
          externalSystems: [
...
test("[UT][regression-suite][regression_execution_profile_export_writer_ps1] exportExecutionProfilePs1 includes sensitive warning when includeResolvedSecrets=true", async () => {
  ...
  try {
    ...
    await writeExecutionProfileExport({
      ...
      planRuns: [
        { order: 1, planName: "plan-a", status: "executed", runStatus: "fail", runId: "run-a" },
      ],
    });
...
test("[UT][regression-suite][regression_execution_profile_export_writer_ps1] exportExecutionProfilePs1 keeps secrets redacted when only sessionExport includeResolvedSecrets is set", async () => {
  ...
  try {
    ...
    assert.match(
      script,
      /Join-Path \$script:McpJvmExportScriptDir 'scripts\\keycloak-token-bootstrap\\refresh-keycloak-token\.ps1'/,
    );
