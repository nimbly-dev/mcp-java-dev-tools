      ...
      targets: [
        {
          type: "class_method",
          selectors: {
            fqcn: "org.example.VisitsController",
            method: "listVisits",
      ...
      steps: [
        {
          ...
          transport: {
            http: {
              method: "POST",
              pathTemplate: "/owners/1/pets/1/visits",
              body: { date: "2026-01-01", description: "regression visit" },
            },
          },
          expect: [
            {
    ...
    writeJson(path.join(planRoot, "contract.json"), {
      targets: [
        {
          type: "class_method",
          selectors: {
            fqcn: "org.example.FavoritesController",
            method: "create",
            sourceRoot: "src/main/java",
          },
        },
      ...
      steps: [
        ...
        },
        {
          ...
          protocol: "http",
          transport: {
            http: { method: "GET", url: "http://localhost:8082/favorites/${favoriteId}" },
          },
          expect: [
            {
              id: "ok",
        ...
        if (calls === 1) {
          return {
            structuredContent: {
              ...
              durationMs: 10,
              body: largeBody,
              bodyPreview: largeBody.slice(0, 2048),
            },
          };
        }
        return {
      ...
      steps: [
        {
          ...
          protocol: "http",
          transport: {
            http: { method: "POST", pathTemplate: "/events", body: { kind: "created" } },
          },
    ...
    writeJson(path.join(planRoot, "contract.json"), {
      ...
      steps: [
        ...
        },
        {
          ...
          protocol: "http",
          transport: {
            http: { method: "GET", url: "http://localhost:8082/events/${triggeredEventId}" },
          },
          expect: [
            {
              id: "ok",
      ...
      targets: [
        {
          type: "class_method",
          selectors: {
            fqcn: "org.example.CourseController",
            method: "list",
            sourceRoot: "src/main/java",
          },
          runtimeVerification: {
            strictProbeKey: "org.example.CourseController#list:10",
        ...
        },
        {
          type: "class_method",
          selectors: {
            fqcn: "org.example.ReviewController",
            method: "list",
            sourceRoot: "src/main/java",
          },
          runtimeVerification: {
            strictProbeKey: "org.example.ReviewController#list:20",
...
test("[UT][regression-suite][execute_regression_plan] executeRegressionPlanWorkflow treats optional-only transport-failure step as non-required in run status", async () => {
  ...
  try {
    ...
    if (out.status === "executed") {
      ...
      assert.equal(out.executionResult.steps[0].status, "pass");
      assert.equal(
        out.executionResult.steps[0].assertions.every(
...
test("[UT][regression-suite][execute_regression_plan] executeRegressionPlanWorkflow does not fail overall run for optional-only transport-failure mismatch", async () => {
  ...
  try {
    ...
    if (out.status === "executed") {
      ...
      assert.equal(out.executionResult.steps[0].status, "fail_assertion");
      assert.equal(
        out.executionResult.steps[0].assertions.some(
        ...
        if (calls === 1) {
          return {
            structuredContent: {
              ...
              durationMs: 8,
              bodyPreview:
                '{"names":[{"locale":"*","value":"Test"},{"locale":"en","value":"Test EN"}]}',
            },
          };
        }
        return {
  ...
  try {
    ...
    if (out.status === "executed") {
      ...
      const correlation = JSON.parse(
        fs.readFileSync(String(out.artifacts.correlationPathAbs), "utf8"),
      );
...
test("[UT][regression-suite][execute_regression_plan] executeRegressionPlanWorkflow fails closed when watcher response cannot be normalized", async () => {
  ...
  try {
      ...
      steps: [
        {
          ...
          extract: [
            {
              from: "response.bodyJson.eventId",
              as: "eventId",
              required: true,
              scope: "suite",
              secret: false,
    ...
    if (out.status === "executed") {
      ...
      assert.equal(
        out.executionResult.watchers?.[0]?.reasonMeta?.providerReasonCode,
        "watcher_response_normalization_failed",
      );
      assert.equal(
        out.executionResult.watchers?.[0]?.reasonMeta?.cause,
        "response_body_json_invalid",
      );
        ...
        if (String(request.method) === "POST") {
          ...
          return {
            structuredContent: {
              ...
              statusCode: 202,
              durationMs: 1,
              body: '{"eventId":"evt-500"}',
              bodyPreview: '{"eventId":"evt-500"}',
            },
          };
        }
        watcherCalls += 1;
...
test("[UT][regression-suite][execute_regression_plan] executeRegressionPlanWorkflow marks watcher dependency blocked when dependent step does not pass", async () => {
  ...
  try {
    ...
    if (out.status === "executed") {
      ...
      assert.equal(
        out.executionResult.watchers?.[0]?.reasonMeta?.dependencyStatus,
        "fail_assertion",
      );
    }
  } finally {
...
test("[UT][regression-suite][execute_regression_plan] executeRegressionPlanWorkflow fails closed when watcher target is unreachable and when expectation stays unsatisfied", async () => {
  ...
  try {
    ...
    const unreachablePlanRoot = path.join(
      root,
      ".mcpjvm",
      projectName,
      "plans",
    ...
    if (unreachableOut.status === "executed") {
      ...
      assert.equal(
        unreachableOut.executionResult.watchers?.[0]?.reasonCode,
        "watcher_target_unreachable",
      );
    }

    const expectationPlanName = "watcher-failed-expectation";
    const expectationPlanRoot = path.join(
      root,
      ".mcpjvm",
      projectName,
      "plans",
    ...
    if (expectationOut.status === "executed") {
      ...
      assert.equal(
        expectationOut.executionResult.watchers?.[0]?.reasonCode,
      ...
      externalVerification: [
        {
          ...
          expect: [
            ...
            },
            {
              ...
              actualPath: "response.bodyJson.task.status",
              operator: "field_equals",
              expected: "completed",
            },
          ],
...
test("[UT][regression-suite][execute_regression_plan] executeRegressionPlanWorkflow fails external verification deterministically when expectations are unmet", async () => {
  ...
  try {
    ...
    if (out.status === "executed") {
      ...
      assert.equal(
        out.executionResult.externalVerification?.[0]?.reasonCode,
...
test("[UT][regression-suite][execute_regression_plan] executeRegressionPlanWorkflow blocks external verification deterministically on unresolved placeholders", async () => {
  ...
  try {
    ...
    if (out.status === "executed") {
      ...
      assert.equal(
        out.executionResult.externalVerification?.[0]?.reasonMeta?.missingContextKey,
        "taskId",
      );
    }
  } finally {
...
test("[UT][regression-suite][execute_regression_plan] executeRegressionPlanWorkflow blocks external verification deterministically when target is unreachable", async () => {
  ...
  try {
          ...
          structuredContent:
            callCount === 1
              ...
              : {
                  ...
                  durationMs: 5,
                  reasonCode: "connect_failed",
                  reasonMeta: { url: "http://localhost:8082/tasks/task-123" },
                },
        };
      },
    });
    ...
    if (out.status === "executed") {
      ...
      assert.equal(
        out.executionResult.externalVerification?.[0]?.reasonMeta?.transportReasonCode,
        "connect_failed",
      );
    }
  } finally {
...
test("[UT][regression-suite][execute_regression_plan] executeRegressionPlanWorkflow treats thrown external verification transport errors as runtime failures", async () => {
  ...
  try {
    ...
    if (out.status === "executed") {
      ...
      assert.equal(
        out.executionResult.externalVerification?.[0]?.reasonMeta?.errorMessage,
        "socket hang up",
      );
    }
  } finally {
