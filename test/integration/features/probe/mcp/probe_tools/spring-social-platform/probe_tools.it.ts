...
test("[IT][probe] happy-path covers regression, probe status, capture, class inventory, and batch probe operations", async () => {
  if (!runtime) throw new Error("post-app runtime was not started");
...
test("[IT][probe] protected createPost requires bearer auth and executes with run-as headers", async () => {
  if (!runtime) throw new Error("post-app runtime was not started");
  ...
  assert.equal(
    createdPayload.content,
    "Authenticated createPost request from the MCP integration flow.",
  );
...
test("[IT][probe] actuate forces deterministic fixture branch outcomes for the same update request", async () => {
  if (!runtime) throw new Error("post-app runtime was not started");
...
test("[IT][probe] actuate with unresolved strict line target fails closed during reset/wait and returns to observe", async () => {
  if (!runtime) throw new Error("post-app runtime was not started");
...
test("[IT][probe] arm actuation without returnBoolean fails closed", async () => {
  if (!runtime) throw new Error("post-app runtime was not started");
...
test("[IT][probe] fail-closed paths cover invalid project roots, bad recipe hints, invalid strict lines, and invalid actuation keys", async () => {
  if (!runtime) throw new Error("post-app runtime was not started");
...
test("[IT][probe] inherited controller methods across Spring mappings resolve deterministic request candidates", async () => {
  const expectedByMethod: Record<
    (typeof inheritedMethodHints)[number],
